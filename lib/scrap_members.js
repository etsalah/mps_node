...
async function getMembers (mpCards) {
  ...
  for (const cardIndex in mpCards) {
    ...
    const nDetails = Member.build({
      memberId: memberId, name: name, party: partyValue,
      constituency: constituency, region: region, photo: imgSrcValue,
      profile: profileUrl})

    nDetails.save().catch((err) => {
      console.log("member already present in database")
    })
